#!/usr/bin/env bash
# build_cli_portable.sh — Build portable md_cli Linux binaries that run on
...
#
# Output: cli/dist/md_cli-linux-x64-portable and -arm64-portable.
#
# Usage:
...
mkdir -p dist
rm -f dist/md_cli-linux-x64-portable dist/md_cli-linux-arm64-portable

run_pkg() {
  npm exec --yes --package="$PKG_VERSION" -- pkg "$@"
}

echo "==> Building dist/md_cli-linux-x64-portable (${PKG_TARGET}-linux-x64) with $PKG_VERSION"
run_pkg . --targets "${PKG_TARGET}-linux-x64" --output dist/md_cli-linux-x64-portable

echo "==> Building dist/md_cli-linux-arm64-portable (${PKG_TARGET}-linux-arm64) with $PKG_VERSION"
run_pkg . --targets "${PKG_TARGET}-linux-arm64" --output dist/md_cli-linux-arm64-portable

chmod 0755 dist/md_cli-linux-x64-portable dist/md_cli-linux-arm64-portable

echo ""
echo "==> Verifying glibc compatibility:"
for f in dist/md_cli-linux-x64-portable dist/md_cli-linux-arm64-portable; do
...
./dist/md_cli-linux-x64-portable version 2>&1 || true

echo ""
echo "==> Built:"
ls -lh dist/md_cli-linux-*-portable 2>/dev/null
