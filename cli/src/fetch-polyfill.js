...
//
// We implement only the subset api.js uses for non-upload calls:
...
// FormData / Blob are NOT polyfilled — `upload`, `assets upload`, and
// `folder-assets upload` still require Node 18+ (use the modern build,
// not the portable one). Login, list, get, share, etc. all work.
    ...
    return new Promise((resolve, reject) => {
      ...
      if (init.body !== undefined && init.body !== null) {
        if (Buffer.isBuffer(init.body)) {
          ...
          // FormData / Blob — not supported in this polyfill.
          reject(new Error(
            'multipart upload requires Node 18+; use the modern md_cli build (not the portable one)'
          ));
          return;
        }
