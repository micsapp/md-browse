#!/usr/bin/env bash
# install.sh — Download the right md_cli binary for this machine and
# install it as `md_cli` on $PATH. Detects OS, arch, and (on Linux) glibc
...
# Defaults
# ---------------------------------------------------------------------------
BASE_URL="${MD_CLI_BASE_URL:-https://tnas_d.micsapp.com/s/md_cli}"
BIN_NAME="${MD_CLI_BIN_NAME:-md_cli}"
PREFIX=""
FORCE_USER=0
UNINSTALL=0
FORCE_PORTABLE=0
...
detect_target() {
  ...
  case "$kernel" in
    Linux)  OS=linux  ;;
    Darwin) OS=macos  ;;
    CYGWIN*|MINGW*|MSYS*) die "Windows isn't supported by this installer. Download md_cli-win-x64.exe from ${BASE_URL} directly and place it on your PATH." ;;
  ...
  if [[ "$OS" == linux ]]; then
    ...
    if [[ "$VARIANT" == portable ]]; then
      ASSET="md_cli-linux-${ARCH}-portable"
    else
      ASSET="md_cli-linux-${ARCH}"
    fi
  else
    VARIANT=modern
    ASSET="md_cli-${OS}-${ARCH}"
  fi
}
...
main_install() {
  ...
  # macOS so it's always available. Also strip com.apple.quarantine in case
  # a downloader set it.
  if [[ "$OS" == macos ]]; then
...
Next steps:
  $BIN_NAME login --url https://md.example.com
  $BIN_NAME list
  $BIN_NAME help
MSG
}
